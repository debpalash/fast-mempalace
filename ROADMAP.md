...
Path from current Zig rewrite to **full drop-in parity** with the upstream Python `mempalace` package, then to **outmatching** it with capabilities the Python stack cannot reach.
...
Core engine that executes the four pillars faster than upstream while holding 80–93% less RAM (see `BENCHMARK.md`).

- [x] `init` / `stats` — palace DB bootstrap + pragma mapping
- [x] `mine <path> [wing]` — concurrent directory ingestion
...
- [x] `mempalace.yaml` drop-in config (backwards compatible)
- [x] Native Metal / CUDA embedding via statically linked `llama.cpp`
- [x] MIT license + GitHub Actions CI matrix (macOS, Ubuntu)

---

## Phase 1 — Full Parity (v0.2)

Close every remaining gap with the upstream `pip install mempalace` surface. Each item blocks the "100% drop-in" marketing claim.

> ⚠ Audit of upstream CLI still pending (PyPI fetch blocked in current env). These items are inferred from project structure + typical memory-tool surface. Cross-check before cutting v0.2.

- [ ] **`mempalace mine` flag parity** — `--wing`, `--room`, `--recursive`, `--ignore`, `--dry-run`
- [ ] **`mempalace search` flag parity** — `--limit`, `--wing`, `--format=json|md|plain`, similarity threshold
- [ ] **`mempalace init` vs `stats`** — upstream names one `init`; alias our `stats` → `init` where appropriate
- [ ] **Incremental re-mining** — content-hash skip for unchanged files (currently re-embeds all)
- [ ] **Delete / forget** — `mempalace forget <id|wing>` to evict drawers
- [ ] **Export / import** — `mempalace export <path>` JSONL dump + re-ingest round-trip
- [ ] **Ignore-pattern parity** — `.gitignore`-style globs matching upstream semantics exactly
- [ ] **Config schema audit** — every upstream yaml key respected or explicitly rejected with diagnostic
...
**Definition of done:** a user can `pip uninstall mempalace && cp ./mempalace /usr/local/bin/` and every script in their pipeline still works unchanged.
...
- [ ] **Compile-time schema** — Zig comptime validation of mempalace.yaml; bad config fails at build, not runtime

### v0.4 — Reach Beyond CLI

- [ ] **Watch mode** — `mempalace watch <path>` file-system events → auto re-mine (upstream Python blocks on ChromaDB lock; we don't)
- [ ] **Embedded HTTP API** — `mempalace serve --port 8080` pure Zig handler, <5 MB RAM overhead
...
- [ ] **Homebrew formula** — `brew install mempalace-zig`
- [ ] **Prebuilt release binaries** — GH Releases: darwin-arm64, darwin-x86_64, linux-x86_64, linux-arm64
...
- [ ] **Plugin SDK** — stable `lib/mempalace.h` C ABI for 3rd-party languages
